...
class TaskRunner:
    ...
    def run(self, config):
        ...
        if config.actor_rollout_ref.actor.strategy == 'fsdp':
            ...
            from verl_patch.workers.code.fsdp_workers import (
            ...
            if config.actor_rollout_ref.rollout.mode in ("async_vllm", "async_agent"):
        ...
        # Reward Manager (Scheme A)
        reward_fn_kwargs = {
            'tokenizer': tokenizer,
            'num_examine': 5,
            'compute_score': compute_score,
        }
        # Optional extended kwargs if supported by the manager
        try:
        ...
        try:
            ...
            val_reward_fn = reward_manager_cls(**val_fn_kwargs)

        resource_pool_manager = ResourcePoolManager(resource_pool_spec=resource_pool_spec, mapping=mapping)

        trainer = RayKernelTrainer(
            config=config,
            tokenizer=tokenizer,
            processor=processor,
